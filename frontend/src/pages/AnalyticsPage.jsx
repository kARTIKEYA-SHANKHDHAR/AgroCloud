...
const AnalyticsPage = ({ mode }) => {
  ...
  const { labels, irrigationSeries, noIrrigationSeries } = useMemo(() => {
    ...
    predictions.forEach((p) => {
      ...
      if (p.raw === 1) daily[day].irrigationNeeded += 1;
  ...
  const { soilLabels, soilValues, soilIrrigationRates } = useMemo(() => {
    ...
    predictions.forEach((p) => {
      ...
      if (p.raw === 1) irrigations[soil] = (irrigations[soil] || 0) + 1;
    });
  ...
  // Summary stats
  const irrigationNeededCount = predictions.filter((p) => p.raw === 1).length;
  const noIrrigationCount     = predictions.filter((p) => p.raw !== 1).length;
  const irrigationRate =
    predictions.length > 0
      ? Math.round((irrigationNeededCount / predictions.length) * 100)
      : 0;
  ...
  const avgTemp =
    validPredictions.length > 0
      ? (validPredictions.reduce((s, p) => s + Number(p.temperature), 0) / validPredictions.length).toFixed(1)
      : "—";
  const avgHumidity =
    validPredictions.length > 0
      ? (validPredictions.reduce((s, p) => s + Number(p.humidity), 0) / validPredictions.length).toFixed(1)
      : "—";
    ...
    datasets: [
      {
        ...
        borderColor: ["#f59e0b", "#22c55e", "#38bdf8", "#a855f7", "#ef4444"],
        borderWidth: 1,
        borderRadius: 6,
      },
    ],
      ...
      {hasPredictions && (
        <>
          ...
          <div className="mb-6 grid gap-4 md:grid-cols-4">
            <StatCard
              label={t.anaStatTotal}
              value={predictions.length}
              hint={t.anaStatTotalHint}
            />
            <StatCard
              label={t.anaStatRate}
              value={`${irrigationRate}%`}
              hint={t.anaStatRateHint}
              tone={irrigationRate > 60 ? "warning" : "success"}
            />
            <StatCard
              label={t.anaStatAvgTemp}
              value={`${avgTemp}°C`}
              hint={t.anaStatAvgTempHint}
            />
            <StatCard
              label={t.anaStatAvgHumid}
              value={`${avgHumidity}%`}
              hint={t.anaStatAvgHumidHint}
            />
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            {cropLabels.length > 0 && (
              <ChartCard
                title={t.anaCropTitle}
                description={t.anaCropDesc}
                type="bar"
                data={cropChartData}
              />
            )}
            ...
            {soilLabels.length > 0 && (
              <ChartCard
                title={t.anaSoilTitle}
                description={t.anaSoilDesc}
                type="bar"
                data={soilChartData}
              />
            )}
              ...
              <div className="space-y-3">
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-amber-600 dark:text-amber-300 font-medium">
                      {t.anaIrrigationNeeded}
                    </span>
                    <span className="text-gray-500 dark:text-slate-400">
                      {irrigationNeededCount} ({irrigationRate}%)
                    </span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-gray-500 dark:text-slate-400">
                      {noIrrigationCount} ({100 - irrigationRate}%)
                    </span>
                  </div>
                  <div className="h-2 rounded-full bg-gray-100 dark:bg-slate-700 overflow-hidden">
                    <div
